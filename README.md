...
[上表](#助詞)提到，`に`前邊多數都係「對像」、「時間點」、「接觸點」、「變化終點」、「位置點」、「移動動詞終點」，其實呢一堆都係講緊「一點」，就好似上邊張圖咁，`に`前邊嘅物件係紅心，而句子中被移到紅心嘅物件或人係支鏢。

其他看似例外，但其實都係同一把戲，好似表「選擇、決定」時用嘅`に`：
```
カレーライスにします。//揀咖哩飯
```
...
```
無錯，3條都係答`に`，分別代表「對於」、「對於」同「作為」，睇返上邊`なります`前嘅`に`係代表「成為」，思考下「對於」、「作為」、「成為」3樣野，其實係中文入邊都係指向用嘅字，就正正係「把心」同「鏢」嘅概念啦，


不過，「表示目的」、「表示比例、均等分配」時用嘅`に`，似乎就唔可以當佢係「一點」，真係要記一記啦。
